...
// Menulis file dan isinya ke dalam folder data
fs.writeFile('./data/log.txt', payload, (err, data) => {
    if (err) {
        console.error(err);
    }
    console.log(color.green.bold('Data berhasil disimpan!'));
})

// Membaca file dan isinya yang telah dibuat
fs.readFile('./data/log.txt', 'utf-8', (err, data) => {
    if (err) {
        console.error(err);
    }
    console.log(color.yellow.bold('Data berhasil read : '), data);
})
